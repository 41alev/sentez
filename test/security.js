...
(async () => {
  ...
  ok('yüklenen dosyanın gerçek bayt imzası doğrulanıyor', /verifyUploadedFile/.test(docSrc) && /File content does not match/.test(docSrc));
  ...
  const validPng = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg==', 'base64');
  fdUpload.append('file', new Blob([validPng], { type: 'image/png' }), 'zararli.php');
  ...
  ok('tehlikeli uzantılı ama geçerli PNG içerikli dosya kabul ediliyor', uploadRes.status === 201, JSON.stringify(uploaded));
  ...
  const fdSpoof = new FormData();
  fdSpoof.append('file', new Blob([Buffer.from('<html><script>alert(1)</script></html>')], { type: 'image/png' }), 'sahte.png');
  fdSpoof.append('docType', 'other');
  const spoofRes = await fetch(BASE + '/api/documents', {
    method: 'POST', headers: { Authorization: `Bearer ${admin}` }, body: fdSpoof
  });
  ok('PNG olarak bildirilen HTML içeriği reddediliyor', spoofRes.status === 415, `got ${spoofRes.status}`);
