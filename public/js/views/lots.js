const ViewLots = (() => {
  ...
  let state = { page: 1, q: '', status: '', warehouseId: '', itemId: '', expiringDays: '' };
  let warehouses = [];

  async function render(el, params = {}) {
    Object.assign(state, params, { page: 1 });
  ...
  async function load(el) {
    let res;
    try { res = await Api.lots({ ...state, pageSize: 25 }); } catch (e) { UI.err(e); return; }

    el.innerHTML = `
      <div class="topbar">
        <div><h2>${t('lotsTitle')}</h2><div class="sub">${t('lotsSub')} · ${res.total}</div></div>
        <div class="topbar-actions">
          <div class="search-box">${UI.icon(UI.ICONS.search)}<input id="lqS" placeholder="${t('search')}" value="${esc(state.q)}"></div>
      ...
      <div class="filters">
        ${select('lStatus', [{ v: '', l: t('all') + ' — ' + t('status') },
          ...
          { v: 'blocked', l: t('blocked') }, { v: 'rejected', l: t('rejected') }], state.status)}
        ${select('lWh', [{ v: '', l: t('all') + ' — ' + t('warehouse') }, ...warehouses.map(w => ({ v: w.id, l: w.name }))], state.warehouseId)}
        ${select('lExp', [{ v: '', l: t('all') },
          { v: '30', l: UI.getLang() === 'tr' ? '30 gün içinde SKT' : 'Expiring in 30 days' },
          { v: '0', l: t('expired') }], state.expiringDays)}
      </div>
      ...
      <div class="card">
        ${table([
          ...
          { key: 'receivedAt', label: t('receivedAt'), render: r => ts(r.receivedAt), cls: 'nowrap' },
    ...
    document.getElementById('lStatus').onchange = e => { state.status = e.target.value; state.page = 1; load(el); };
    document.getElementById('lWh').onchange = e => { state.warehouseId = e.target.value; state.page = 1; load(el); };
    document.getElementById('lExp').onchange = e => { state.expiringDays = e.target.value; state.page = 1; load(el); };
    ...
    el.querySelectorAll('[data-trace]').forEach(b => b.onclick = () => traceDialog(b.dataset.trace));
    el.querySelectorAll('[data-status]').forEach(b => b.onclick = () => statusDialog(el, res.data.find(x => x.id === b.dataset.status)));
    el.querySelectorAll('[data-transfer]').forEach(b => b.onclick = () => transferDialog(el, res.data.find(x => x.id === b.dataset.transfer)));
  }

  function statusDialog(el, lot) {
    modal({
  ...
  function transferDialog(el, lot) {
    modal({
  ...
  /* ---------- traceability ---------- */
  async function traceDialog(lotId) {
    modal({ title: t('traceability'), size: 'xwide', body: loading(), footer: `<button class="btn btn-ghost" data-close>${t('close')}</button>` });
    let back, fwd, recall;
